...
import { fetchTranscriptForInput, normalizeChapterRules } from "./tools/fetch-transcript.js"
import { validateChapterToolInput } from "./tools/validate.js"
...
export interface YouTubeChaptersPluginOptions {
  chapterRules?: string[]
  glossary?: ChapterGlossary
}

export const YouTubeChaptersPlugin: Plugin = async (_input, options = {}) => {
  const chapterRules = normalizeChapterRules(options.chapterRules)
  ...
  return {
    tool: {
      youtube_chapters_fetch_transcript: tool({
        ...
        async execute(args) {
          const result = await fetchTranscriptForInput(args, undefined, { chapterRules, glossary })
          return {
      ...
      youtube_chapters_validate: tool({
        ...
        args: {
          ...
          glossary: tool.schema.record(tool.schema.string(), tool.schema.string()).optional().describe("Optional deterministic title corrections returned by youtube_chapters_fetch_transcript."),
        },
        async execute(args) {
          const result = validateChapterToolInput(args, glossary)
          return {
